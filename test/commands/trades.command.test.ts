...
describe('test trades sync command', () => {
  ...
  test('expect to response be instance of TradesResponse', async () => {
    const result = await conn.getTrades(false);
    expect(result).toBeInstanceOf(TradesResponse);
  });
});
